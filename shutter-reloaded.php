...
class sh_reloadedClass {
    var $srel_options, $srel_main, $srel_included, $srel_excluded, $do_footer;

function sh_reloadedClass() {
    $this->srel_options = get_option('srel_options');
    $this->srel_main = get_option('srel_main');
    $this->srel_included = get_option('srel_included');
    $this->srel_excluded = get_option('srel_excluded');
}
...
function makeshutter() { 
    ...
    case 'srel_pages' :
        $included = (array) $this->srel_included;
        if ( in_array( $post->ID, $included ) )
            $addshutter = true;
        break;
    
    case 'auto_set' :
        $excluded = (array) $this->srel_excluded;
...
function shutterOnload(func){var oldonload=window.onload;if(typeof window.onload!='function'){window.onload = func;}else{window.onload=function(){func();oldonload();}}}
<?php       } else { ?>
...
<?php       } else {
    ...
    if ( document.links[i].className.toLowerCase().indexOf('shutter') != -1 ) {
<?php           } ?>
      var shtype = document.links[i].href.slice(-4).toLowerCase();
      if ( shtype == '.jpg' || shtype == '.png' || shtype == '.gif' || shtype == 'jpeg' ) {
        shutterLinks[i] = document.links[i].href;
...
<?php           if ( $this->srel_main == 'srel_class' || $this->srel_main == 'auto_set' ) echo "    }\n"; 
            } ?>
  }
});
//]]>
...
<?php 
if( $this->srel_options['custom'] == 1 ) { ?>
<style type="text/css">
<?php 
    if( $this->srel_options['shcolor'] != '000000' || $this->srel_options['opacity'] != '80' ) { ?>
...
function srel_optpage() {
    ...
    $def = array( 'shcolor' => '000000', 'opacity' => '80', 'capcolor' => 'ffffff', 'btncolor' => '999999', 'waitcolor' => 'ae0a0a' );
    ...
    if ( isset($_POST['srel_saveopt']) ) {
        ...
        $new_srel_options['btncolor'] = preg_match("/[0-9A-Fa-f]{6}/", $_POST['btncolor']) ? strtolower($_POST['btncolor']) : '999999';
        $new_srel_options['waitcolor'] = preg_match("/[0-9A-Fa-f]{6}/", $_POST['waitcolor']) ? strtolower($_POST['waitcolor']) : 'ae0a0a';
        $new_srel_options['opacity'] = preg_match("/^[0-9][0-9]?$/", $_POST['opacity']) ? $_POST['opacity'] : '80';
        
        if( $def != $new_srel_options )
            $new_srel_options['custom'] = 1;
...
<?php   } else {
            ...
            if( ! empty($excluded) ) {
                sort($excluded);
                update_option('srel_excluded', $excluded);
            } else delete_option('srel_excluded');
        }
    }
...
<?php   } else {
            ...
            if( ! empty($included) ) {
                sort($included);
                update_option('srel_included', $included);
            } else delete_option('srel_included');
        }
    }
...
value="Show Help" style="color:blue;" /></p>
    ...
    <p>&lowast; To edit the text "LOADING" and "Click to Close", change the values near the top of <a href="<?php bloginfo('wpurl'); ?>/wp-admin/templates.php?file=wp-content/plugins/shutter-reloaded/shutter-reloaded.js">shutter-reloaded.js</a></p>
    </div>
...
<?php
    ...
    onclick="if (form.srel_exclude.value == ''){alert('Please enter the ID that you want to remove from the excluded list.');return false;}" />
...
if ( $opt == 'srel_class' ) { ?>
    ...
    <p><strong>Shutter is activated for all links pointing to an image, that have class = &quot;shutter&quot;, &quot;shutterset&quot; or &quot;shutterset_123&quot;</strong></p>

    <p>Class = &quot;shutter&quot; will display a single image, class = &quot;shutterset&quot; will create a single set for all images and class = &quot;shutterset_123&quot;, where 123 is an unique number, will create multiple sets on the same page.</p>
...
?>
    ...
    <p class="submit"><input type="submit" name="srel_saveopt" value="Save Options" /></p>
...
function srel_addmenu() {
    if ( function_exists('add_options_page') ) {
	   add_options_page('Shutter Reloaded', 'Shutter Reloaded', 9,  __FILE__, 'srel_optpage');
    }
}
...
if ( class_exists("sh_reloadedClass") ) {
    ...
    add_action('wp_head', array(&$sh_reloaded, 'makeshutter') );
    add_action('get_footer', array(&$sh_reloaded, 'addfooter') );
